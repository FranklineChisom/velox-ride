...
import { 
  Loader2, User, MapPin, Heart, Plus, Trash2, ChevronRight, FileText, 
  AlertCircle, CheckCircle, LogOut, CreditCard, Bell, Smartphone, Mail
} from 'lucide-react';
  ...
  return (
    <section className="bg-white border border-slate-100 rounded-[2rem] p-6 shadow-sm mb-6">
      ...
      <div className="space-y-3">
            ...
            <div key={p.id} className="flex justify-between items-center p-3 border border-slate-100 bg-slate-50 rounded-xl">
               <div className="overflow-hidden">
                   <div className="font-bold text-sm text-slate-900 truncate flex items-center gap-2">
                       {p.label}
                       {['home','work'].includes(p.label.toLowerCase()) && <span className="bg-blue-100 text-blue-700 text-[9px] px-2 py-0.5 rounded-full uppercase">Shortcut</span>}
                   </div>
         ...
         {places.length === 0 && !isAdding && <p className="text-sm text-slate-400 italic text-center py-4">No places saved yet.</p>}
         {isAdding && (
            <div className="space-y-2 animate-fade-in p-4 bg-slate-50 rounded-2xl border border-slate-200">
               <input className="w-full p-3 border rounded-xl text-sm outline-none focus:border-black" placeholder="Label (e.g. Home, Gym)" value={newPlace.label} onChange={e => setNewPlace({...newPlace, label: e.target.value})} />
               <LocationInput value={newPlace.address} onChange={val => setNewPlace({...newPlace, address: val})} placeholder="Search Address" />
                   ...
                   <div className="flex items-center gap-6 w-full">
                      ...
                      <div className="flex-1">
                         ...
                         <div className="flex items-center gap-2 mt-2">
                            {profile?.is_verified ? <span className="text-[10px] bg-green-100 text-green-700 px-3 py-1 rounded-full font-bold flex items-center gap-1 w-fit"><CheckCircle className="w-3 h-3"/> Verified</span> : <span className="text-[10px] bg-slate-100 text-slate-600 px-3 py-1 rounded-full font-bold">Basic Account</span>}
                         </div>
                      </div>
                   </div>
